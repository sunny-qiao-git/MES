...
public partial class AppMes_Import_frmImportUpload : BasePage
{
    ...
    protected void DyUpload(DataTable datatable)
    {
    ...
    /// <returns></returns>
    public static DataTable ExcelToDataTable(string filePath, string IsXls, int sheetNo)
    {
        ...
        using (var file = new FileStream(filePath, FileMode.Open, FileAccess.Read))
        {
            if (IsXls == ".xlsx")
            {
                ...
                var sheet = hssfworkbook.GetSheetAt(sheetNo);
                for (var j = 0; j < 100; j++)
                {
